...
public class FlowTagLayout extends ViewGroup {
    ...
    public List<String> strLis = new ArrayList<>();
    ...
    public void reloadData(int index) {
        ...
        for (int i = 0; i < mAdapter.getCount(); i++) {
            ...
            if (childView instanceof LinearLayout) {
                ...
                if (strLis.size() > 0) {
                    tv.setBackgroundColor(ContextCompat.getColor(mContext, R.color.line_5));
                    tv.setTextColor(ContextCompat.getColor(mContext,R.color.line_4));
                    childView.setEnabled(false);
                    for (String str : strLis) {
                        if (str.equals(tv.getText().toString())) {
                            tv.setBackground(ContextCompat.getDrawable(mContext,R.drawable.selector_2));
                            tv.setTextColor(ContextCompat.getColorStateList(mContext,R.color.selector_color));
//                            tv.setTextColor(ContextCompat.getColor(mContext,R.color.selector_color));
...
//              new MarginLayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT));//这个构造方法所然能使用但是编译器会报错
    ...
    public void selectItem(int index) {
        if (mAdapter != null) {
            for (int k = 0; k < mAdapter.getCount(); k++) {
                mCheckedTagArray.put(k, k == index ? true : false);
                getChildAt(k).setSelected(k == index ? true : false);
                //            View childView = mAdapter.getView(index, null, this);
                //            childView.setSelected(true);
            }
        }
    }
     ...
     * @param index
     */
    public void selectItemWithCancel(int index) {
        if (mAdapter != null) {
            boolean isHas = false;
            int pos = -1;
            isSelect = false;
            for (int i = 0; i < mCheckedTagArray.size(); i++) {
                if (mCheckedTagArray.get(i) && index == i) {
                    isHas = true;
                    pos = i;
                    break;
                }
            ...
            for (int k = 0; k < mAdapter.getCount(); k++) {
                ...
                if (pos == -1) {
                    ...
                    getChildAt(k).setSelected(k == index ? true : false);
                    if (k == index) {
                        selectStr = mAdapter.getItem(k).toString();
                        isSelect = true;
                    }
    ...
    public void unSelectItemAll() {
        for (int k = 0; k < mAdapter.getCount(); k++) {
            mCheckedTagArray.put(k, false);
            getChildAt(k).setSelected(false);
//            View childView = mAdapter.getView(k, null, this);
//            childView.setSelected(false);
        }
    }
